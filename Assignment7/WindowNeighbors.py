...
def windowNN(train, rec, max_dist, dist_func=None):
    if dist_func is None:
        dist_func = lambda a, b: np.linalg.norm(a - b)

    neighbors = [r for r in train if dist_func(r[:-1], rec) <= max_dist]

    return max(set(np.array(neighbors)[:,-1]), key=list(np.array(neighbors)[:,-1]).count)


def kernelDensityNN(train, rec, sim_func=None):
    m, n = np.shape(train)
    y_values = set(train[:,-1])
    probs = {}
    for y in y_values:
        training_values = np.array([r for r in train if r[-1] == y])
        # m_c = len(training_values)
        bayes_prob = 0
        for r in training_values:
            bayes_prob += sim_func(rec, r[:-1])
        # bayes_prob /= m_c
        probs[y] = bayes_prob/m
...
if __name__ == "__main__":
    ...
    #     if windowNN(training_data, t[:-1], 2.5) == t[-1]:
    #         test_acc += 1.0/m
    #
    # print "2.5-Radius Nearest Neighbors Spambase Testing Accuracy: ", test_acc
    ...
    #     if windowNN(train_ecoc_table, t, 0.83, dist_func=lambda x, y: (1 - (np.dot(x,y)/(np.linalg.norm(x)*np.linalg.norm(y))))) != test_labels[ind]:
    #         test_acc += 1.0/m
    #
    # print "0.83 Nearest Neighbors Digits Testing Accuracy: ", test_acc

    # test_acc = 0.0
    # for ind, t in enumerate(test_ecoc_table):
    #     if kernelDensityNN(train_ecoc_table, t, sim_func=lambda x, y: (np.dot(x,y)**2)) == test_labels[ind]:
    #         test_acc += 1.0/m
    #
    # print "Kernel Density Nearest Neighbors Digits Testing Accuracy: ", test_acc
    ...
    # test_acc = 0.0
    # for t in testing_data:
    #     if kernelDensityNN(training_data, t[:-1], sim_func=lambda x, y: (np.exp(-(np.linalg.norm(x-y)**2)))) == t[-1]:
    #         test_acc += 1.0/m
    #
    # print "Kernel Density Nearest Neighbors Spambase Testing Accuracy: ", test_acc
